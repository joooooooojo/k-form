...
const clientType = (formValue?: T): KFormItemProps => {
  return {
    rowKey: "row-1",
    label: "客户类型",
    prop: "type",
...
```vue
<KForm v-model="baseForm" :options="root"></KForm>
```
...
| `options`                  | `Array<KFormItemProps>` or `() => Arrayable<KFormItemProps>` | 表单项配置                                                                                                                                           |
...
| `label`         | `string`                                                                          | 文本标签                                                                                              |
| `width`         | `string` or `number`                                                              | 表单项宽度                                                                                            |
| `labelWidth`    | `string` or `number`                                                              | 标签宽度                                                                                              |
...
| `show`          | `boolean` or `((formValue: T) => boolean)`                                        | 是否展示 默认展示，传入函数时formValue为表单数据                                                      |
| `payload`       | `any`                                                                             | 载荷，不同表单项类型有不同载荷，使用时手动指定特定类型                                                |
| `slotName`      | `string`                                                                          | 插槽                                                                                                  |
| `next`          | `((formValue?: any, currentKey?: string) => Arrayable<KFormItemProps>)` or `null` | 动态表单项 -- 下一项                                                                                  |
